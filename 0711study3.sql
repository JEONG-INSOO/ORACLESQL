...
--6. 영업팀' 부서에서 일하는 사원의 이름, 연락처, 주소를 보이시오. (단 연락처 없으면 '연락처 없음' , 연락처 끝4자리 중 앞2자리는 별표* 로 표시하시오. (예시. 010-111-**78)
  select name "이름", nvl(phoneno,'연락처없음') "연락처" , address "주소", t2.deptname "부서"
...
--12. 3명 이상의 사원이 있는 부서의 사원 이름을 보이시오. 
  select name "사원이름", deptname "부서이름"
    from employee t1, department t2
   where t1.deptno = t2.deptno
...
--15. EXISTS 연산자로 '빅데이터 구축' 프로젝트에 참여하는 사원의 이름을 보이시오. 
  select t1.name "사원 이름", t2.projname "프로젝트 명"
...
--17. 부서별로 급여가 부서평균 급여 보다 높은 사원의 이름과 월급을 보이시오. 
  select t1.deptname "부서 이름", t2.name "사원이름", t2.salary "급여"
    from department t1,
         employee t2 right outer join (select deptno, avg(salary) avg_salary 
                                         from employee 
                                     group by deptno) t3
